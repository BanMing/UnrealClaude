	...
	const FString& Description)
{
	...
	// where the per-script confirmation is the dominant friction. Default ON for this fork (see
	// Config/DefaultEditor.ini) to keep autonomous flows unblocked; flip to false in DefaultEditor.ini
	// to restore the human-in-the-loop dialog. Every auto-approval is logged so the audit trail survives.
